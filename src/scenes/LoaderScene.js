...
export class LoaderScene extends Scene {
    ...
    preload() {
        ...
        this.load.atlas(
            'character-good-staff',
            '/assets/final/goodstaffsprite.png',
            '/assets/final/goodstaffsprite.json'
        );
        this.load.atlas(
            'character-bad-staff',
            '/assets/final/badstaffsprite.png',
            '/assets/final/badstaffsprite.json'
        );

        this.load.image('cat', '/assets/final/chaoscat.png');
        this.load.atlas(
            'cat-squish',
            '/assets/final/chaoscatsquish.png',
            '/assets/final/chaoscatsquish.json'
        );
        ...
        STAT_TYPES.forEach((type) => {
            for (let i = 1; i <= 5; i++) {
                if (i <= 3) {
                    this.load.image(
                        `${type}-ORB-${i}`,
                        `/assets/final/${type}/${type}-ORB-${i}.png`
                    );
                }
